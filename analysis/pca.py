...
class PCA(DimensionalityReducer):
    ...
    def perform_pca(_self, n_components: int = 2, columns: List[str] = None, 
        ...
        try:
            ...
            # 분석할 열 선택
            if columns is None:
                columns = _self.numeric_cols
            else:
                # 존재하지 않는 열 제외
                columns = [col for col in columns if col in _self.df.columns]
                # 수치형이 아닌 열 제외
                columns = [col for col in columns if pd.api.types.is_numeric_dtype(_self.df[col])]
            
            if not columns:
                raise ValueError("분석할 수치형 열이 없습니다.")
            
            # 결측치 처리
            data = _self.df[columns].fillna(_self.df[columns].median())
            ...
            _self.reduced_data = pca.fit_transform(scaled_data)
            _self.components = pca.components_
            _self.explained_variance = pca.explained_variance_ratio_
            _self.reduction_model = pca
            ...
            # 범주형 열 추가
            categorical_cols = _self.df.select_dtypes(exclude=['number']).columns
            if not categorical_cols.empty:
                for col in categorical_cols:
                    pca_df[col] = _self.df[col].values
            ...
            if 'pca' not in _self.execution_times:
                _self.execution_times['pca'] = []
            _self.execution_times['pca'].append(execution_time)
    ...
    @st.cache_data(ttl=3600) # Added cache decorator as it's likely intended
    def get_pca_biplot_data(self, n_components: int = 2, columns: List[str] = None,
        ...
        # PCA 수행
        # Since perform_pca now takes _self as its first argument,
        # and this method is part of the same class, we call it as _self.perform_pca
        pca_df, explained_variance, loadings = self.perform_pca(n_components=n_components, columns=columns, scaler_type=scaler_type)
    ...
    @st.cache_data(ttl=3600) # Added cache decorator
    def get_pca_feature_importance(_self) -> pd.DataFrame:
        ...
        """
        if _self.components is None or _self.explained_variance is None:
            raise ValueError("먼저 perform_pca 메서드를 호출하세요.")
        
        # 열 이름 가져오기 (스케일링 시 사용한 열)
        feature_names = _self.numeric_cols
        
        # 각 주성분에 대한 변수 기여도 계산
        # 각 변수의 로딩을 해당 주성분의 설명된 분산 비율로 가중합하여 중요도 계산
        n_components = len(_self.explained_variance)
        importance = np.zeros(len(feature_names))
        
        for i in range(n_components):
            importance += np.abs(_self.components[i, :]) * _self.explained_variance[i]
        
        # 결과 정규화
        importance = importance / importance.sum() if importance.sum() != 0 else importance # Avoid division by zero
        ...
        return result_df
    @st.cache_data(ttl=3600) # Added cache decorator
    def get_optimal_n_components(_self, threshold: float = 0.95, max_components: int = None) -> Tuple[int, np.ndarray, pd.DataFrame]:
        ...
        """
        # 분석할 수치형 열
        numeric_df = _self.df[_self.numeric_cols].dropna()
        
        # 데이터 스케일링
        scaled_data = _self._scale_data(_self.numeric_cols)
        
        # 최대 주성분 수 설정
        if max_components is None:
            max_components = min(len(_self.numeric_cols), len(numeric_df))
        else:
            max_components = min(max_components, len(_self.numeric_cols), len(numeric_df))
    ...
    @st.cache_data(ttl=3600) # Added cache decorator
    def get_component_interpretation(_self, n_top_features: int = 5) -> pd.DataFrame:
        ...
        DataFrame
            각 주성분별 주요 변수 및 로딩 값
        """
        if _self.components is None or _self.explained_variance is None: # Check explained_variance too
            raise ValueError("먼저 perform_pca 메서드를 호출하세요.")
        
        feature_names = _self.numeric_cols
        n_components = _self.components.shape[0]
        
        results = []
        
        for i in range(n_components):
            # i번째 주성분의 로딩
            component_loadings = _self.components[i, :] # Renamed to avoid conflict
            ...
            for idx in top_indices:
                results.append({
                    'component': f'PC{i+1}',
                    'explained_variance': _self.explained_variance[i],
                    'feature': feature_names[idx],
                    'loading': component_loadings[idx],
                    'loading_abs': abs(component_loadings[idx]),
                    'direction': 'positive' if component_loadings[idx] > 0 else 'negative'
                })
        ...
        return result_df
    # This method primarily calls other methods and then uses matplotlib, so caching it directly might not be
    # as beneficial unless get_optimal_n_components is very slow and its inputs don't change often.
    def plot_variance_explained(_self, max_components: int = None) -> plt.Figure:
        ...
        """
        # 최적 주성분 개수 탐색
        _, cum_variance, variance_df = _self.get_optimal_n_components(max_components=max_components)
